...
import { GlobeIcon, SparklesIcon, PaintBrushIcon, EqualizerIcon } from '../constants';
...
const TabButton: React.FC<TabButtonProps> = ({ icon, label, isActive, onClick }) => (
    <button
        onClick={onClick}
        className={`flex-1 flex items-center justify-center gap-2 px-3 py-2 text-sm font-semibold rounded-md transition-all duration-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2 focus-visible:ring-offset-gray-800 ${
            isActive
...
type SettingsTab = 'display' | 'audio' | 'language';
...
export const SettingsPanel: React.FC<SettingsPanelProps> = ({ isOpen, onClose }) => {
  ...
  return (
    ...
    >
      <div 
        className="relative bg-gray-800 text-white rounded-2xl shadow-2xl p-6 w-full max-w-lg m-4 transform transition-all animate-fade-in-up overflow-y-auto max-h-[90vh]"
      ...
      >
        ...
        <div className="flex gap-2 mb-6 bg-gray-900/70 p-1 rounded-lg">
            <TabButton
                icon={<PaintBrushIcon className="w-5 h-5" />}
                label={t('settings_tab_display')}
                isActive={activeTab === 'display'}
                onClick={() => setActiveTab('display')}
            />
            <TabButton
                icon={<EqualizerIcon className="w-5 h-5" />}
                label={t('settings_tab_audio')}
